...
Contracts established on `main`; backend control plane implemented, tested and green in CI on
`backend`.
...
**On `backend`** (pushed, branch `backend`): the control plane. 21 test suites, 245 tests, all
seven quality gates green locally and all five CI jobs green on GitHub.
...
Backend, locally: `21 suites, 245 tests, 0 failures`; format, lint, typecheck and build clean.

Backend CI, run `34168080823` on `backend` — **all five jobs green**:
...
The suites run twice: against in-process PGlite, and against a real PostgreSQL 17 service
container. Full output in `HANDOFF.md` on the `backend` branch.
...
- Three real bugs were found and fixed in the backend by its own security suite; see `HANDOFF.md`
  on `backend`. The most serious: reuse detection revoked the token family *inside* a transaction
  and then threw, which rolled the revocation back — so a detected refresh-token theft left the
  attacker's freshly issued token working.
- **`drizzle-orm` < 0.45.2 carries a HIGH advisory** (GHSA-gpj5-g38j-94v9, SQL injection via
  improperly escaped identifiers). Backend is on 0.45.2 and `npm audit --omit=dev` reports zero
  vulnerabilities. **Any other domain that adopts Drizzle must pin ≥ 0.45.2.**
...
- ~~The backend's production `pg` driver path is unverified.~~ **Closed** — CI's `postgres` matrix
  leg now runs every suite against a real PostgreSQL 17 service container and passes.
...
1. ~~Tag `protocol-v1.0.0`.~~ **Done.** PR #1 merged as `83dfcc9`, and `protocol-v1.0.0` now points
   at that commit. Every domain can pin it:

   ```
   git fetch origin --tags
   git archive protocol-v1.0.0 contracts | tar -x -C .contracts/
   ```

   Please review the tag placement — if you would rather it sat elsewhere on `main`, move it now,
   before other domains pin against it.

2. **Domain CI workflows must not all be named `.github/workflows/ci.yml`.** GitHub keys a
   workflow by its **file path**, so every domain using that path collapses into one workflow
   record and inherits whichever name registered first — backend runs were appearing in the
   Actions UI as "Frontend CI". `backend` now uses `backend-ci.yml`; `frontend`, `mobile` and
   `design` should do the same, or per-branch CI independence (`DEV_GUIDE.md` §6) is cosmetic
   only.

3. **`DEV_GUIDE.md` §3.4's feature-branch convention cannot work.** Git refuses
